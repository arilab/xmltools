  ...
  */
      ...
      if (childNodes[i].nodeType == 1) {
        ...
        if (nodeName == "xs:complexContent") {
          ...
          if (extensionNode) {
			...
			var extensionChildNode = GetFirstChildNode(extensionNode, "xs:sequence");
			if (!extensionChildNode)
			  extensionChildNode = GetFirstChildNode(extensionNode, "xs:choice");
			if (extensionChildNode != null) {
			  previousIndex = nodeIndex;
              nodeIndex = AddNode(extensionChildNode, parentIndex, previousIndex);
              if (parentIndex != null && previousIndex == null)
                Nodes[parentIndex].firstChildIndex = nodeIndex;
              if (previousIndex != null)
                Nodes[previousIndex].nextIndex = nodeIndex;
  ...
  function ParseSequenceNode(node, parentIndex)
  {
    ...
    for (i = 0; i < childNodes.length; i++) {
      if (childNodes[i].nodeType == 1 && childNodes[i].nodeName != "xs:annotation") {
        previousIndex = nodeIndex;
        nodeIndex = AddNode(childNodes[i], parentIndex, previousIndex);
        if (parentIndex != null && previousIndex == null)
          Nodes[parentIndex].firstChildIndex = nodeIndex;
  ...
  */
  ...
  function ResolveNamespaces(schemaNode)
  {
    for(var i = 0; i < schemaNode.attributes.length; i++) {
	  var attribute = schemaNode.attributes[i];
	  if (attribute.name.startsWith("xmlns:")) {
	    var namespace = new Object();
		namespace.prefix = attribute.name.substring(6);
  ...
  function LoadComplexTypes(node, namespace, excludedNodeName)
  {
    var nodeCount = null;
    var childNodes = node.childNodes;
    var i = null;
    ...
    for (i = 0; i < childNodes.length; i++)
      if (childNodes[i].nodeType == 1 && childNodes[i].nodeName == "xs:complexType") {
		var childNodeName = childNodes[i].getAttribute("name");
        var complexType = new Object();
		var prefix = "";
		if (namespace) {
		  if (excludedNodeName != null && childNodeName == excludedNodeName)
			  prefix = "";
		  else
		    prefix = namespace.prefix + ":";
		}
        complexType.name = prefix + childNodes[i].getAttribute("name");
        var mynode = new Object();
  ...
  function LoadSimpleTypes(node, namespace, excludedNodeName)
  {
    var nodeCount = null;
    var childNodes = node.childNodes;
    var i = null;
    ...
    for (i = 0; i < childNodes.length; i++)
      if (childNodes[i].nodeType == 1 && childNodes[i].nodeName == "xs:simpleType") {
        ...
        simpleType.name = prefix + childNodeName;
        var mynode = new Object();
  ...
  function LoadSelf(schemaNode, rootNodeName)
  {
	var targetNamespace = schemaNode.getAttribute("targetNamespace");
	var namespace = FindNamespaceByName(targetNamespace);
	LoadSimpleTypes(schemaNode, namespace, rootNodeName);
    tracePar.innerHTML = "Simple types loaded: " + SimpleTypes.length + " - loading complex types of main schema file ...";

    LoadComplexTypes(schemaNode, namespace, rootNodeName);
  ...
  function LoadIncludes(sourcePath, node)
  {
    ...
    for (i = 0; i < childNodes.length; i++)
	  if (childNodes[i].nodeType == 1 && (childNodes[i].nodeName == "xs:include" || childNodes[i].nodeName == "xs:import")) {
        schemaLocation = childNodes[i].getAttribute("schemaLocation");
		name = childNodes[i].getAttribute("namespace");
		namespace = FindNamespaceByName(name);
        if (schemaLocation) {
		  ...
		  */
		  xmlDoc = HttpGet(schemaURL);
          nIncludes++;

          schemaNode = GetFirstChildNode(xmlDoc, "xs:schema");
          LoadSimpleTypes(schemaNode, namespace, null);
          LoadComplexTypes(schemaNode, namespace, null);
        }
      }
  ...
  function FindNamespaceByPrefix(prefix) {
...
*/
  ...
  function LoadSchema()
  {
    ...
    var xsdFilename = GetParam("xsd") + ".xsd";
	var rootName = GetParam("root");
    var complexType = new Object();
    ...
    try {
	  if (xsdFilename && rootName) {
		xmlDoc = HttpGet(xsdFilename);
	  } else {
	...
	ResolveNamespaces(schemaNode);
	
	var rootNode;
	if (xsdFilename && rootName) {
      var rootElement = "<xs:element name='" + rootName + "' type='" + rootName + "'><xs:annotation><xs:documentation>Root node</xs:documentation></xs:annotation></xs:element>";
	  rootNode = GetFirstChildNode(parser.parseFromString(rootElement, "text/xml"), "xs:element");
	} else {
      rootNode = GetFirstChildNode(schemaNode, "xs:element");
	}
	
	var sourcePath = xmlDoc.URL.replace(/[^/]*$/, '');
